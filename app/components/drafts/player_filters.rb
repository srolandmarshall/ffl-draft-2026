...
class Components::Drafts::PlayerFilters < Components::Base
    ...
    div(class: "flex flex-wrap items-center gap-1.5 sm:gap-2", aria: { label: "Filter by position" }) do
      ...
      Player::POSITIONS.each do |position|
        label(class: "cursor-pointer") do
          input(type: "checkbox", name: "positions[]", value: position, checked: @positions.include?(position), class: "peer sr-only", data: { draft_filter_target: "position", action: "change->draft-filter#scheduleSelection" })
  ...
  def team_filter
    ...
    details(id: "#{frame_id}-team-filter", class: "relative w-full self-start sm:w-auto", data: { turbo_permanent: true, draft_filter_target: "teamMenu" }) do
      ...
      div(class: "absolute inset-x-0 top-full z-20 mt-2 grid max-h-64 w-full grid-cols-2 gap-x-4 gap-y-1 overflow-y-auto rounded-xl border border-white/15 bg-slate-950 p-3 shadow-2xl shadow-black/40 sm:left-auto sm:right-0 sm:w-auto sm:min-w-56") do
        div(class: "col-span-2 sticky top-0 z-10 -mx-3 -mt-3 mb-1 flex items-center justify-between border-b border-white/10 bg-slate-950 px-3 py-2") do
